...
class Safe extends WebLoginBase{
	...
	//新手任务
	public final function xinren(){
		$this->display('safe/xinren.php');
	}
	/**
	 * 密码管理
	 */
	...
	public final function setCBAccount(){
		...
		$update['add_sheng']=$_POST['add_sheng'];
		$update['add_shiqu']=$_POST['add_shiqu'];
		
		$ress=$this->getRow("select * from {$this->prename}bank_list where id=? LIMIT 1", $update['bankId']);
		$update['code']=$ress['code'];
		...
		if($bank=$this->getRow("select * from {$this->prename}member_bank where uid=? LIMIT 1", $this->user['uid'])){
